...
output_path = Path(sys.argv[0]).resolve().parent.parent / f'{dataset_name}_output'
if output_path.exists():
    shutil.rmtree(output_path)


report_path = Path(sys.argv[0]).resolve().parent.parent / f'{dataset_name}_report'
if report_path.exists():
    shutil.rmtree(report_path)


db_path = Path(sys.argv[0]).resolve().parent.parent / f'{dataset_name}.db'
if db_path.exists():
